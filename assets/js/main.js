...
(function ($) {
    ...
    var hoverImgCursor = function () {
        ...
        $(".hover-cursor-img").on("mousemove", function (e) {
            ...
            let imgWidth = hoverImage.outerWidth() || 200;
            let imgHeight = hoverImage.outerHeight() || 250;
            let left = e.clientX + offsetX;
            let top = e.clientY + offsetY;

            if (left + imgWidth > window.innerWidth - 15) {
                left = e.clientX - imgWidth - offsetX;
            }
            if (top + imgHeight > window.innerHeight - 15) {
                top = window.innerHeight - imgHeight - 15;
            }
            if (top < 15) {
                top = 15;
            }
    ...
    ---------------------------------------------------------- */
    $(".award-item.hover-cursor-img").on("click", function () {
        var imgEl = $(this).find(".award_img_cert img");
        if (imgEl.length === 0) return;
        var src = imgEl.attr("src");
        var alt = imgEl.attr("alt") || "Certificate";
        $("#cert-lightbox-img").attr("src", src).attr("alt", alt);
        $("#cert-lightbox").css("display", "flex");
    ...
    $(document).on("keydown", function (e) {
        if (e.key === "Escape") {
            $("#cert-lightbox").css("display", "none");
    ...
    var textRotate = function () {
        if ($(".wg-curve-text").length > 0) {
            if ($(".text-rotate").length > 0) {
                const text = "design - development - growth - since 2016 -";
